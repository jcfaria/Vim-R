...
function CheckVimcomVersion()
    ...
    let flines += ['vim_r_home <- "' . g:rplugin.home . '"',
                \ 'libs <- c(' . libs . ')']
...
function RInitExit(...)
    ...
    call AddForDeletion(g:rplugin.localtmpdir . "/libs_in_nrs_" . $VIMR_ID)
    call AddForDeletion(g:rplugin.tmpdir . "/libnames_" . $VIMR_ID)
    if len(s:RWarn) > 0
...
function StartNServer()
    ...
    if exists("g:R_local_R_library_dir")
        let nrs_path = FindNCSpath(g:R_local_R_library_dir . '/vimcom')
    else
        if filereadable(g:rplugin.compldir . '/vimcom_info')
            let info = readfile(g:rplugin.compldir . '/vimcom_info')
            if len(info) == 3
                " Update vimcom information
                let g:rplugin.vimcom_info = {'version': info[0], 'home': info[1], 'Rversion': info[2]}
                let g:rplugin.debug_info['vimcom_info'] = g:rplugin.vimcom_info
                let nrs_path = FindNCSpath(info[1])
            else
                call delete(g:rplugin.compldir . '/vimcom_info')
                call RWarningMsg("ERROR in vimcom_info! Please, do :RDebugInfo for details.")
                return
            endif
        else
            call RWarningMsg("ERROR: vimcom_info not found. Please, run :RDebugInfo for details.")
            return
        endif
    endif

    let ncspath = substitute(nrs_path, '/vimrserver.*', '', '')
    let ncs = substitute(nrs_path, '.*/vimrserver', 'vimrserver', '')
    ...
    if g:R_objbr_opendf
        let $VIMR_OPENDF = "TRUE"
    endif
    if g:R_objbr_openlist
        let $VIMR_OPENLS = "TRUE"
    endif
    if g:R_objbr_allnames
        let $VIMR_OBJBR_ALLNAMES = "TRUE"
    endif
    let $VIMR_RPATH = g:rplugin.Rcmd
    ...
    " let g:rplugin.jobs["Server"] = StartJob(['valgrind', '--log-file=/dev/shm/vimrserver_valgrind_log', '--leak-check=full', ncs], g:rplugin.job_handlers)
    if has('win32')
        call UnsetRHome()
    endif
...
function s:BuildAllArgs(...)
    ...
    let rscrpt = ['library("vimcom", warn.conflicts = FALSE)']
    for afile in flist
        if filereadable(afile) == 0
            let pkg = substitute(substitute(afile, ".*/args_", "", ""), "_.*", "", "")
            let rscrpt += ['vimcom:::vim.buildargs("' . afile . '", "' . pkg . '")']
        endif
    endfor
    if len(rscrpt) == 1
        return
...
function UpdateSynRhlist()
    ...
    let g:rplugin.libs_in_nrs = readfile(g:rplugin.localtmpdir . "/libs_in_nrs_" . $VIMR_ID)
...
if exists("*WaitVimComStart")
    echohl WarningMsg
    call input("Please, uninstall Vim-R-plugin before using Vim-R. [Press <Enter> to continue]")
    echohl None
endif

let s:ff = split(globpath(&rtp, "R/functions.vim"), '\n')
if len(s:ff) > 1
    function WarnDupVimR()
        ...
        unlet ff
        let msg  += ["", "Please, uninstall one version of Vim-R.", ""]
    ...
    if v:vim_did_enter
        ...
        autocmd VimEnter * call WarnDupVimR()
    endif
endif
unlet s:ff
...
if exists('g:R_commented_lines')
    call RWarningMsg('R_commented_lines no longer exists. See: https://github.com/jalvesaq/Vim-R/issues/743')
endif
...
" 2025-08-08
if exists('g:R_nvimpager') && !exists('g:R_vimpager')
    let g:R_vimpager = g:R_nvimpager
endif
if exists('g:R_nvim_wd') && !exists('g:R_vim_wd')
    let g:R_vim_wd = g:R_nvim_wd
endif
