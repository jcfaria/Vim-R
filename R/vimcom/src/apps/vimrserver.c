...
static void SaveWinPos(char *cachedir) {
    ...
    if (!GetWindowRect(VimHwnd, &rcV)) {
        fprintf(stderr, "Could not get Vim position\n");
        fflush(stderr);
        return;
    }
...
static void ArrangeWindows(char *cachedir) {
    ...
    if ((fgets(b, 31, f))) {
        ...
        fprintf(stderr, "Error reading Vim left position\n");
        fflush(stderr);
        fclose(f);
        return;
    }
    if ((fgets(b, 31, f))) {
        ...
        fprintf(stderr, "Error reading Vim top position\n");
        fflush(stderr);
        fclose(f);
        return;
    }
    if ((fgets(b, 31, f))) {
        ...
        fprintf(stderr, "Error reading Vim right position\n");
        fflush(stderr);
        fclose(f);
        return;
    }
    if ((fgets(b, 31, f))) {
        ...
        fprintf(stderr, "Error reading Vim bottom position\n");
        fflush(stderr);
        fclose(f);
        return;
    }
    ...
    if (rcV.left > 0 && rcV.top > 0 && rcV.right > 0 && rcV.bottom > 0 &&
        ...
        if (!SetWindowPos(VimHwnd, HWND_TOP, rcV.left, rcV.top, rcV.right,
                          rcV.bottom, 0)) {
            fprintf(stderr, "Error positioning Vim window\n");
            fflush(stderr);
        }
    }
...
{
    // Set the value of VimHwnd
    if (getenv("WINDOWID")) {
#ifdef _WIN64
        VimHwnd = (HWND)atoll(getenv("WINDOWID"));
#else
        ...
        if (!VimHwnd)
            VimHwnd = FindWindow(NULL, "nvim");
        if (!VimHwnd)
            VimHwnd = FindWindow(NULL, "vim");
        if (!VimHwnd) {
            fprintf(stderr, "Neither \"Neovim\" nor \"Vim\" window found\n");
            fflush(stderr);
        }
    }
}
...
static int run_R_code(const char *s, int senderror) {
    ...
    char b[1024];
    snprintf(b, 1023, "VIMR_TMPDIR=%s", getenv("VIMR_REMOTE_TMPDIR"));
    putenv(b);
    snprintf(b, 1023, "VIMR_COMPLDIR=%s", getenv("VIMR_REMOTE_COMPLDIR"));
    putenv(b);
    snprintf(b, 1023,
    ...
    snprintf(b, 1023, "VIMR_TMPDIR=%s", tmpdir);
    putenv(b);
    snprintf(b, 1023, "VIMR_COMPLDIR=%s", compldir);
    putenv(b);
...
static void finish_bol() {
    ...
    // Message to the editor: Update both syntax and Rhelp_list
...
void update_pkg_list(char *libnms) {
    ...
    if (libnms) {
        ...
        snprintf(buf, 511, "%s/libnames_%s", tmpdir, getenv("VIMR_ID"));
...
static void init(void) {
    ...
    if (getenv("VIMR_OPENDF"))
        OpenDF = 1;
    else
        OpenDF = 0;
    if (getenv("VIMR_OPENLS"))
        OpenLS = 1;
    else
        OpenLS = 0;
    if (getenv("VIMR_OBJBR_ALLNAMES"))
        allnames = 1;
    else
        allnames = 0;
        ...
        if (strcmp(s, wrd) == 0) {
            ...
            if (f[1][0] == '\003' && str_here(f[4], "[\x12not_checked\x12]")) {
                snprintf(compl_buffer, 1024,
                         "E%svimcom:::vim.GlobalEnv.fun.args(\"%s\")\n",
                         getenv("VIMR_ID"), wrd);
                send_to_vimcom(compl_buffer);
                return;
            }
...
void stdin_loop() {
    ...
    while (fgets(line, 1023, stdin)) {
        ...
        msg = line;
        switch (*msg) {
        case '1': // Start server and wait vimcom connection
            start_server();
            Log("server started");
            break;
        case '2': // Send message
