...
# version 0.4-11 (released on 2012-01-04) and adapted to vimcom.
...
gbRd.args2txt <- function(pkg = NULL, rdo, arglist) {
    ...
    get_items <- function(a, rdo) {
        ...
        .Call("get_section", res, "arguments", PACKAGE = "vimcom")
    }
...
vim.fix.string <- function(x, edq = TRUE) {
    x <- gsub("\n", "\\\\n", x)
    x <- gsub("\r", "\\\\r", x)
    x <- gsub("\t", "\\\\t", x)
    x <- gsub("'", "\x13", x)
...
#' @param edq Whether double quotes should be escaped.
vim.args <- function(funcname, txt = "", pkg = NULL, objclass, extrainfo = FALSE, edq = TRUE) {
    # Adapted from: https://stat.ethz.ch/pipermail/ess-help/2011-March/006791.html
    if (!exists(funcname))
        return("")
    frm <- NA
    funcmeth <- NA
    if (!missing(objclass) && vim.grepl("[[:punct:]]", funcname) == FALSE) {
    ...
    if (pkgname[1] != ".GlobalEnv" && extrainfo && length(frm) > 0) {
        ...
        arglist <- lapply(arglist, vim.fix.string, edq)
    }
    ...
    for (field in names(frm)) {
        ...
        if (extrainfo) {
            ...
            if (type == "symbol") {
                ...
                str2 <- paste0(" = \x12, \x12menu\x12: \x12\"", vim.fix.string(frm[[field]]), "\"\x12")
                ...
                str2 <- paste0(" = \x12, \x12menu\x12: \x12",
                               vim.fix.string(deparse(frm[[field]]), FALSE), "\x12")
            } else {
                str2 <- paste0("\x12, \x12menu\x12: \x12 \x12")
            }
            ...
            if (type == "symbol") {
                ...
                res <- append(res, paste0("[\x12", field, "\x12], "))
                warning(paste0("vim.args: ", funcname, " [", field, "]", " (typeof = ", type, ")"))
            }
        }
    }
...
#' @param x Object name
vim.getInfo <- function(printenv, x) {
    info <- NULL
    als <- VimcomEnv$pkgdescr[[printenv]]$alias[VimcomEnv$pkgdescr[[printenv]]$alias[, "name"] == x, "alias"]
    try(info <- VimcomEnv$pkgdescr[[printenv]]$descr[[als]], silent = TRUE)
    if (length(info) == 1)
        return(info)
    return("\006\006")
}
...
vim.omni.line <- function(x, envir, printenv, curlevel, maxlevel = 0) {
    if (curlevel == 0) {
        ...
        if (x.group == "f") {
            if (curlevel == 0) {
                if (vim.grepl("GlobalEnv", printenv)) {
                    ...
                    info <- vim.getInfo(printenv, x)
                    cat(n, "\006\003\006\006", printenv, "\006",
                        vim.args(x, pkg = printenv), info, "\006\n", sep = "")
                }
            } else {
                ...
                if (info == "\006\006") {
                    ...
                    if (!inherits(xattr, "try-error"))
                        if (!is.null(xattr) && length(xattr) == 1)
                            info <- paste0("\006\006", .Call("rd2md", xattr, PACKAGE = "vimcom"))
                }
                cat(n, "\006", x.group, "\006", x.class, "\006", printenv,
                    "\006[]", info, "\006\n", sep = "")
            }
    ...
    if ((is.list(xx) || is.environment(xx)) && curlevel <= maxlevel) {
        ...
        if (!is.null(xxl) && xxl > 0) {
            for (k in obj.names) {
                vim.omni.line(paste(x, "$", k, sep = ""), envir, printenv, curlevel, maxlevel)
            }
        }
...
GetFunDescription <- function(pkg) {
    ...
    GetDescr <- function(x) {
        x <- paste0(x, collapse = "")
        ttl <- .Call("get_section", x, "title", PACKAGE = "vimcom")
        dsc <- .Call("get_section", x, "description", PACKAGE = "vimcom")
        x <- paste0("\006", ttl, "\006", dsc)
        x
    }
    VimcomEnv$pkgdescr[[pkg]] <- list("descr" = sapply(pkgInfo, GetDescr),
                                       "alias" = als)
}
...
vim.buildargs <- function(afile, pkg) {
    ...
    for (obj in obj.list) {
        ...
        arglist <- lapply(arglist, vim.fix.string)
        cat(obj, "\006",
...
vim.bol <- function(omnilist, packlist, allnames = FALSE) {
    vim.OutDec <- getOption("OutDec")
    on.exit(options(vim.OutDec))
    options(OutDec = ".")

    if (!missing(packlist) && is.null(VimcomEnv$pkgdescr[[packlist]]))
    ...
    for (curpack in listpack) {
        ...
        if (l > 0) {
            ...
            for (obj in obj.list) {
                ol <- try(vim.omni.line(obj, curpack, curlib, 0))
...
vim.buildomnils <- function(p) {
    if (length(p) > 1) {
        n <- 0
        for (pkg in p)
            n <- n + vim.buildomnils(pkg)
        if (n > 0)
            return(invisible(1))
        return(invisible(0))
    }
    # No verbosity because running as Vim job
    ...
    if (need_build) {
        ...
        vim.bol(paste0(bdir, "omnils_", p, "_", pvi), p, TRUE)
        return(invisible(1))
    }
    return(invisible(0))
}
